function iniciar_ajax_historico_personas_creadas(){
  /*LIMPIAR FORMULARIO*/
  ...
  var boton_limpiar_fechas_historico_personas_creadas = document.getElementById('limpiar_fechas_historico_personas_creadas');
  ...
  boton_limpiar_fechas_historico_personas_creadas.addEventListener('click', limpia_formulario_de_fechas_personas_creadas, false);
  ...
  function limpia_formulario_de_fechas_personas_creadas(e) {
    //aqui buscamos el formulario de fechas y lo reseteamos
    document.getElementById('fechas_inicio_fin_historico_personas_creadas').reset();
  }
  /*LIMPIAR FORMULARIO*/
  ...
  var exportar_a_excel_historico_personas_creadas = document.getElementById('exportar_excel_historico_personas_creadas');
  //despues vamos a ponerlo a la escucha del evvento click para que ejecute la funcion de inicializar_ajax_busqueda_general_historico_personas
  exportar_a_excel_historico_personas_creadas.addEventListener('click', inicializar_ajax_busqueda_general_historico_personas, false);
  /*EXPORTACION A EXCEL*/
  ...
  var boton_busqueda_general_historico_personas_creadas = document.getElementById('boton_busqueda_general_historico_personas_creadas');
  ...
  boton_busqueda_general_historico_personas_creadas.addEventListener('keyup', inicializar_ajax_busqueda_general_historico_personas, false);
  ...
  function inicializar_ajax_busqueda_general_historico_personas(e) {
    ...
    var fecha_inicio_historico_creacion_personas = document.getElementById('fecha_inicio').value;
    //declaramos las variables con el supuesto valor que tengan, esta es la de fecha fin
    var fecha_fin_historico_creacion_personas = document.getElementById('fecha_fin').value;
    ...
    if (fecha_inicio_historico_creacion_personas && fecha_fin_historico_creacion_personas) {
      //aqui estamos sumandole las horas minutos y segundos a las variables, solamente si tienen la fecha
      fecha_inicio_historico_creacion_personas = fecha_inicio_historico_creacion_personas +" "+ "0" +":"+ "00" +":"+ "00";
      //aqui estamos sumandole las horas minutos y segundos a las variables, solamente si tienen la fecha
      fecha_fin_historico_creacion_personas = fecha_fin_historico_creacion_personas +" "+ "23" +":"+ "59" +":"+ "59";
      ...
      if (e.target.id == "exportar_excel_historico_personas_creadas") {
        //aqui hacemos la nueva data que se le enviara como parametro a la funcion de envio a AJAX
        data = `fecha_inicio=${fecha_inicio_historico_creacion_personas}&fecha_fin=${fecha_fin_historico_creacion_personas}&query_general=${boton_busqueda_general_historico_personas_creadas.value}&exportacion_a_excel=si`;
        ...
        window.location='busqueda_historico_personas_creadas/?' + data
      }else{
        ...
        data = `fecha_inicio=${fecha_inicio_historico_creacion_personas}&fecha_fin=${fecha_fin_historico_creacion_personas}&query_general=${e.target.value}`;
        //luego de la obtencion de los datos, llamaremos a la funcion de envio de datos por AJAX a Django
        enviar_datos_AJAX_historico_personas_creadas(data);
        console.log(data)
      }
    ...
    //de lo contrario entonces solamente buscara con los parametros de letras indicadas en el input de busqueda general
      ...
      if (e.target.id == "exportar_excel_historico_personas_creadas"){
        ...
        data = `query_general=${boton_busqueda_general_historico_personas_creadas.value}&exportacion_a_excel=si`;
        //luego aqui llamamos un HREF para que se nos abra la URL y le pasamos los aprametros, para que haga la descarga de una vez, no es necesario pasarlo por ajax, ya que no se va a recargar la apgina solo se va a descar una archivo
        window.location='busqueda_historico_personas_creadas/?' + data
      }else{
  ...
  var peticionHTTP_busqueda_historico_personas_creadas;
  ...
  var tbody_a_actualizar_historico_personas_creadas = document.getElementById('tbody_historico_personas_creadas');
  ...
  var form_historico_creacion_personas = document.getElementById('fechas_inicio_fin_historico_personas_creadas');

  //aqui vamos a comenzar a crear con la variable peticionHTTP_busqueda_historico_personas_creadas, vamos a crear todo lo necesario para el envio y la recepcion de la informacion por AJAX
  ...
  if (window.XMLHttpRequest){
    peticionHTTP_busqueda_historico_personas_creadas = new XMLHttpRequest();
  }else{
    peticionHTTP_busqueda_historico_personas_creadas = new ActiveXObject("Microsoft.XMLHTTP")
  }
  ...
  peticionHTTP_busqueda_historico_personas_creadas.onreadystatechange = function() {
    ...
    if (peticionHTTP_busqueda_historico_personas_creadas.readyState === 4 && peticionHTTP_busqueda_historico_personas_creadas.status === 200) {
      //aqui lo que vamos a hacer es insertar la informacion obtenida en el tbody
      tbody_a_actualizar_historico_personas_creadas.innerHTML = peticionHTTP_busqueda_historico_personas_creadas.response;
  ...
  function enviar_datos_AJAX_historico_personas_creadas(data) {
    ...
    peticionHTTP_busqueda_historico_personas_creadas.open('GET', 'busqueda_historico_personas_creadas/?' + data, true);
    // y con este tercer comando, le estamos diciendo que envie la peticion
    peticionHTTP_busqueda_historico_personas_creadas.send()
  }
  ...
  form_historico_creacion_personas.addEventListener('submit', inicializar_ajax_busqueda_historico_personas, false);
  ...
  function inicializar_ajax_busqueda_historico_personas(e){
    ...
    fecha_inicio_historico_creacion_personas = document.getElementById('fecha_inicio').value +" "+ "0" +":"+ "00" +":"+ "00";
    ...
    fecha_fin_historico_creacion_personas = document.getElementById('fecha_fin').value +" "+ "23" +":"+ "59" +":"+ "59";

    var data = `fecha_inicio=${fecha_inicio_historico_creacion_personas}&fecha_fin=${fecha_fin_historico_creacion_personas}`;
    console.log(data)
...
window.addEventListener('load', iniciar_ajax_historico_personas_creadas, false)
