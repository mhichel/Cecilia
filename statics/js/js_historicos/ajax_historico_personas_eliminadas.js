function iniciar_ajax_historico_personas_eliminadas(){
  /*LIMPIAR FORMULARIO*/
  ...
  var boton_limpiar_fechas_historico_personas_eliminadas = document.getElementById('limpiar_fechas_historico_personas_eliminadas');
  ...
  boton_limpiar_fechas_historico_personas_eliminadas.addEventListener('click', limpia_formulario_de_fechas_personas_eliminadas, false);
  ...
  function limpia_formulario_de_fechas_personas_eliminadas(e) {
    //aqui buscamos el formulario de fechas y lo reseteamos
    document.getElementById('fechas_inicio_fin_historico_personas_eliminadas').reset();
  }
  /*LIMPIAR FORMULARIO*/
  ...
  var exportar_a_excel_historico_personas_eliminadas = document.getElementById('exportar_excel_historico_personas_eliminadas');
  //despues vamos a ponerlo a la escucha del evvento click para que ejecute la funcion de inicializar_ajax_busqueda_general_historico_personas_eliminadas
  exportar_a_excel_historico_personas_eliminadas.addEventListener('click', inicializar_ajax_busqueda_general_historico_personas_eliminadas, false);
  /*EXPORTACION A EXCEL*/
  ...
  var boton_busqueda_general_historico_personas_eliminadas = document.getElementById('boton_busqueda_general_historico_personas_eliminadas');
  ...
  boton_busqueda_general_historico_personas_eliminadas.addEventListener('keyup', inicializar_ajax_busqueda_general_historico_personas_eliminadas, false);
  ...
  function inicializar_ajax_busqueda_general_historico_personas_eliminadas(e) {
    ...
    var fecha_inicio_historico_eliminacion_personas = document.getElementById('fecha_inicio').value;
    //declaramos las variables con el supuesto valor que tengan, esta es la de fecha fin
    var fecha_fin_historico_eliminacion_personas = document.getElementById('fecha_fin').value;
    ...
    if (fecha_inicio_historico_eliminacion_personas && fecha_fin_historico_eliminacion_personas) {
      //aqui estamos sumandole las horas minutos y segundos a las variables, solamente si tienen la fecha
      fecha_inicio_historico_eliminacion_personas = fecha_inicio_historico_eliminacion_personas +" "+ "0" +":"+ "00" +":"+ "00";
      //aqui estamos sumandole las horas minutos y segundos a las variables, solamente si tienen la fecha
      fecha_fin_historico_eliminacion_personas = fecha_fin_historico_eliminacion_personas +" "+ "23" +":"+ "59" +":"+ "59";
      ...
      if (e.target.id == "exportar_excel_historico_personas_eliminadas") {
        //aqui hacemos la nueva data que se le enviara como parametro a la funcion de envio a AJAX
        data = `fecha_inicio=${fecha_inicio_historico_eliminacion_personas}&fecha_fin=${fecha_fin_historico_eliminacion_personas}&query_general=${boton_busqueda_general_historico_personas_eliminadas.value}&exportacion_a_excel=si`;
        ...
        window.location='busqueda_historico_personas_eliminadas/?' + data
      }else{
        ...
        data = `fecha_inicio=${fecha_inicio_historico_eliminacion_personas}&fecha_fin=${fecha_fin_historico_eliminacion_personas}&query_general=${e.target.value}`;
        enviar_datos_AJAX_historico_personas_eliminadas(data);
    ...
    //de lo contrario entonces solamente buscara con los parametros de letras indicadas en el input de busqueda general
      ...
      if (e.target.id == "exportar_excel_historico_personas_eliminadas"){
        ...
        data = `query_general=${boton_busqueda_general_historico_personas_eliminadas.value}&exportacion_a_excel=si`;
        //luego aqui llamamos un HREF para que se nos abra la URL y le pasamos los aprametros, para que haga la descarga de una vez, no es necesario pasarlo por ajax, ya que no se va a recargar la apgina solo se va a descar una archivo
        window.location='busqueda_historico_personas_eliminadas/?' + data
      }else{
  ...
  var peticionHTTP_busqueda_historico_personas_eliminadas;
  ...
  var tbody_a_actualizar_historico_personas_eliminadas = document.getElementById('tbody_historico_personas_eliminadas');
  ...
  var form_historico_creacion_eliminadas = document.getElementById('fechas_inicio_fin_historico_personas_eliminadas');

  //aqui vamos a comenzar a crear con la variable peticionHTTP_busqueda_historico_personas_eliminadas, vamos a crear todo lo necesario para el envio y la recepcion de la informacion por AJAX
  ...
  if (window.XMLHttpRequest){
    peticionHTTP_busqueda_historico_personas_eliminadas = new XMLHttpRequest();
  }else{
    peticionHTTP_busqueda_historico_personas_eliminadas = new ActiveXObject("Microsoft.XMLHTTP")
  }
  ...
  peticionHTTP_busqueda_historico_personas_eliminadas.onreadystatechange = function() {
    ...
    if (peticionHTTP_busqueda_historico_personas_eliminadas.readyState === 4 && peticionHTTP_busqueda_historico_personas_eliminadas.status === 200) {
      //aqui lo que vamos a hacer es insertar la informacion obtenida en el tbody
      tbody_a_actualizar_historico_personas_eliminadas.innerHTML = peticionHTTP_busqueda_historico_personas_eliminadas.response;
  ...
  function enviar_datos_AJAX_historico_personas_eliminadas(data) {
  // le pasaremos por parametro los datos obtenidos de las fechas que serian los parametros para buscar en Django los registros
    ...
    peticionHTTP_busqueda_historico_personas_eliminadas.open('GET', 'busqueda_historico_personas_eliminadas/?' + data, true);
    // y con este tercer comando, le estamos diciendo que envie la peticion
    peticionHTTP_busqueda_historico_personas_eliminadas.send()
  }
  ...
  form_historico_creacion_eliminadas.addEventListener('submit', inicializar_ajax_busqueda_historico_personas, false);

  function inicializar_ajax_busqueda_historico_personas(e){
    ...
    fecha_inicio_historico_eliminacion_personas = document.getElementById('fecha_inicio').value +" "+ "0" +":"+ "00" +":"+ "00";
    ...
    fecha_fin_historico_eliminacion_personas = document.getElementById('fecha_fin').value +" "+ "23" +":"+ "59" +":"+ "59";

    var data = `fecha_inicio=${fecha_inicio_historico_eliminacion_personas}&fecha_fin=${fecha_fin_historico_eliminacion_personas}`;
    console.log(data)
...
window.addEventListener('load', iniciar_ajax_historico_personas_eliminadas, false)
